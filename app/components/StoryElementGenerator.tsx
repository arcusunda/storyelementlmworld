// app/components/StoryElementGenerator.tsx
...
import StoryElementHistory from './StoryElementHistory';
import StoryElementShareButton from './StoryElementShareButton';
...
interface MuertoStoryElementGeneratorProps {
  tokenId?: string;
}

type ElementGenType = 'random' | 'spiritAnimal' | 'itemOfPower';


export default function MuertoStoryElementGenerator({ tokenId: propTokenId }: MuertoStoryElementGeneratorProps) {
  ...
  const [storyElement, setStoryElement] = useState<string>('');
  const [storyElementShortId, setStoryElementShortId] = useState<string>('');
  const [generating, setGenerating] = useState(false);
  const [copied, setCopied] = useState(false);
  const [, setExistingStoryElements] = useState<StoryElementEntry[]>([]);
  const [, setHasExistingStoryElement] = useState(false);
  ...
  const [selectedElementType, setSelectedElementType] = useState<ElementGenType>('random');
  const [currentElementType, setCurrentElementType] = useState<string | undefined>(undefined);
  ...
  const fetchExistingStoryElements = useCallback(async () => {
    if (!tokenId || !address) return;
    
    try {
      const response = await axios.get(`/api/anthropic/storyelement-history?wallet=${address}&tokenId=${tokenId}`);
      
      if (response.data.history && response.data.history.length > 0) {
        setExistingStoryElements(response.data.history);
        setHasExistingStoryElement(true);
        
        const activeStoryElement = response.data.history.find((storyElement: StoryElementEntry) => storyElement.isActive === true);
        
        const storyElementToUse = activeStoryElement || response.data.history[0];
        
        setStoryElement(storyElementToUse.storyElement);
        
        if (storyElementToUse.shortId) {
          setStoryElementShortId(storyElementToUse.shortId);
        }
        
        if (storyElementToUse.animationNotes) {
          setAnimationNotes(storyElementToUse.animationNotes);
        }
        
        if (storyElementToUse.characterName) {
          setFirstName(storyElementToUse.characterName.firstName);
          if (!firstNameOptions.includes(storyElementToUse.characterName.firstName)) {
            setFirstNameOptions(prev => [...prev, storyElementToUse.characterName.firstName]);
          }
        }
        
        if (storyElementToUse.elementType) {
          setCurrentElementType(storyElementToUse.elementType);
        } else {
          setCurrentElementType('random');
        }
      } else {
        setExistingStoryElements([]);
        setHasExistingStoryElement(false);
      }
    } catch (error) {
      console.error('Error fetching existing rawStoryElements:', error);
    }
  ...
  useEffect(() => {
    if (nftData && address) {
      fetchExistingStoryElements();
    }
  }, [nftData, address, fetchExistingStoryElements]);
  ...
  const generateStoryElement = async () => {
    ...
    try {
      ...
      const response = await axios.post('/api/anthropic/storyelement', requestBody);
      
      if (response.data.storyElement) {
        setStoryElement(response.data.storyElement);
        
        if (response.data.shortId) {
          setStoryElementShortId(response.data.shortId);
        }
        ...
        if (response.data.elementType) {
          setCurrentElementType(response.data.elementType);
        }
        
        fetchExistingStoryElements();
      } else {
        console.error('No storyElement in response:', response.data);
        setStoryElement('Failed to generate storyElement. Please try again.');
      }
    } catch (error) {
      console.error('Error generating storyElement:', error);
      
      let errorMessage = 'An error occurred while generating the storyElement. Please try again.';
      ...
      setStoryElement(errorMessage);
    } finally {
      setGenerating(false);
    }
  };

  const handleCopyStoryElement = () => {
    if (storyElement) {
      navigator.clipboard.writeText(storyElement);
      setCopied(true);
  ...
  const renderWalletNotConnected = () => (
    <div className="p-8 text-center bg-gray-800/30 rounded-xl backdrop-blur-sm">
      ...
      <p>Connect your wallet to create a storyElement for your Muerto</p>
    </div>
  );
  ...
  const renderStoryElementContent = () => (
        ...
        <div className="h-full flex items-center justify-center text-gray-500">
          <p>
            {elementLoading 
              ...
              : generating 
                ? "Crafting your Muerto's storyElement..."
                : "Click 'Generate StoryElement' to create a unique StoryElement for your Muerto."
            }
          </p>
        </div>
      )}
  ...
  const renderStoryElementActions = () => {
      ...
      <>
        <div className="mt-3 mb-2 flex items-center">
          <div className="flex-1 text-xs italic text-gray-500 dark:text-gray-400 flex items-center">
            ...
            Note: Muerto Storytime is in Beta. StoryElements are subject to being archived while we refine story quality.
          </div>
        </div>
  
        <div className="mt-2 flex gap-3 justify-end">
          {/* Share Button - Show if storyElement exists */}
          {storyElement && (
            <StoryElementShareButton
              muertosId={parseInt(tokenId || "0", 10)}
              characterName={`${firstName || ""} ${lastName || ""}`}
              storyElement={storyElement}
              shortId={storyElementShortId}
              walletAddress={address || "unknown"}
              className="bg-blue-600 text-white hover:bg-blue-700"
            />
          )}
          <button
            onClick={handleCopyStoryElement}
        ...
        {storyElement && (
          <div className="mt-6 flex flex-col gap-3">
            ...
            >
              ...
              Register Story Element as IP Asset
            </Link>
          </div>
        )}
      </>
  ...
  const renderElementTypeSelector = () => (
    <div className="mb-6 p-4 bg-gray-700/50 rounded-lg">
      <h4 className="text-lg font-medium mb-3 text-purple-300">Story Element Type</h4>
      
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
      <button 
        onClick={() => setSelectedElementType('random')}
        className={`p-3 rounded-lg flex flex-col items-center justify-center border-2 transition-all ${
          selectedElementType === 'random' 
            ? 'border-purple-500 bg-purple-900/30' 
            : 'border-gray-600 bg-gray-800/30 hover:bg-gray-700/30'
        }`}
      >
        <div className="w-10 h-10 mb-2 flex items-center justify-center text-xl">🔮</div>
        <span className="font-medium">Mystical Element</span>
        <p className="text-xs text-gray-400 mt-1">A location, entity, device, event, or other supernatural feature</p>
      </button>
        
        <button 
          onClick={() => setSelectedElementType('spiritAnimal')}
          className={`p-3 rounded-lg flex flex-col items-center justify-center border-2 transition-all ${
            selectedElementType === 'spiritAnimal' 
              ? 'border-purple-500 bg-purple-900/30' 
              : 'border-gray-600 bg-gray-800/30 hover:bg-gray-700/30'
          }`}
        >
          <div className="w-10 h-10 mb-2 flex items-center justify-center text-xl">🐉🦉</div>
          <span className="font-medium">Spirit Animal</span>
          <p className="text-xs text-gray-400 mt-1">A supernatural companion with unique powers</p>
        </button>
        
        <button 
          onClick={() => setSelectedElementType('itemOfPower')}
          className={`p-3 rounded-lg flex flex-col items-center justify-center border-2 transition-all ${
            selectedElementType === 'itemOfPower' 
              ? 'border-purple-500 bg-purple-900/30' 
              : 'border-gray-600 bg-gray-800/30 hover:bg-gray-700/30'
          }`}
        >
          <div className="w-10 h-10 mb-2 flex items-center justify-center text-xl">⚔️🪄</div>
          <span className="font-medium">Item of Power</span>
          <p className="text-xs text-gray-400 mt-1">A magical object with supernatural abilities</p>
        </button>
      </div>
      ...
      {currentElementType && (
        <div className="mt-4 p-2 bg-gray-800/50 rounded border border-gray-700">
          <p className="text-xs text-gray-400">
            Current Element Type: <span className="text-white font-medium">
              {currentElementType === 'spiritAnimal' ? 'Spirit Animal' : 
               currentElementType === 'itemOfPower' ? 'Item of Power' : 
               'Story Element'}
            </span>
          </p>
        </div>
  ...
  return (
    <div className="mt-8 grid grid-cols-1 lg:grid-cols-5 gap-6">
      ...
      {/* Right Column - StoryElement */}
      <div className="lg:col-span-3 storyElement-container rounded-xl p-6">
        <div className="flex items-center justify-between mb-4">
          <div>
            <h3 className="text-xl font-bold flex items-center storyElement-text">
          ...
          >
            {generating ? (
              ...
              <>
                <BookOpen className="mr-2 h-5 w-5" />
                {selectedElementType === 'spiritAnimal' ? 'Generate Spirit Animal' :
                selectedElementType === 'itemOfPower' ? 'Generate Item of Power' :
        ...
        <div className="relative">
          {renderStoryElementContent()}
          {renderStoryElementActions()}
        </div>
      ...
      {storyElement && currentElementType && (
        <div className="mt-6 p-4 bg-gray-800/50 rounded-lg border border-gray-700">
          <h4 className="font-medium text-purple-300 mb-2 flex items-center">
            <Info className="mr-2 h-4 w-4" />
            {currentElementType === 'spiritAnimal' ? 'About Spirit Animals' :
            currentElementType === 'itemOfPower' ? 'About Items of Power' :
            'About Story Elements'}
          </h4>
          <p className="text-sm text-gray-300">
          {currentElementType === 'spiritAnimal' ? 
            'Spirit Animals are supernatural companions that bond with inhabitants of The Violet Reaches. Each has unique abilities that reflect aspects of their bonded character\'s essence.' :
          currentElementType === 'itemOfPower' ? 
            'Items of Power are magical artifacts containing concentrated supernatural energy. Each item grants specific abilities to its wielder and resonates with their personal journey.' :
            'Mystical Elements are tangible features of The Violet Reaches - whether locations, entities, devices, companions, challenges, resources, or events - that your character can directly interact with and that expand their story possibilities.'}
          </p>
        </div>
      )}

      {/* StoryElement History */}
      <div className="lg:col-span-5">
        {nftData && <StoryElementHistory tokenId={nftData.tokenId} />}
      </div>
    </div>
  );
}